"""
Variant of local_multi_model_ae32_relu_structured_prompt.py: same AE backend
(32-dim, fixed nn.ReLU()), same inference pipeline, same 5-field structured
prompt design - adds ONE new field so the operator can answer "when did this
attack happen?", which the prompt2 format could not answer at all.

clean_dataset_dl() (used to build the AE's numeric input) drops the "time"
column entirely - it's a wall-clock timestamp string, not a feature the
autoencoder can reconstruct, so it was correctly excluded from training/
inference input. But select_anomalous_packet() already looks up the matching
row in df_orig (the UNCLEANED raw dataframe) before building the prompt - so
"time" is still available there, just never surfaced. extract_packet_info()
now also reads it, and create_prompt() adds:
  - a "Timestamp" line in the Packet data section (the raw HH:MM:SS.ffffff
    value, e.g. "16:09:04.703364")
  - a new required output field "Time of Occurrence" so the model states it
    explicitly in its answer instead of leaving the operator to go find it
    in a separate log
This does NOT change the AE model, its training data, or its 18-feature
input in any way - "time" is inference-time DISPLAY context only, read from
the raw dataframe, never fed into the autoencoder.

Run standalone with:
    python local_multi_model_ae32_relu_structured_prompt_timing.py --purpose "..." 2>&1 | tee local_multi_model_ae32_relu_structured_prompt_timing/run_$(date +%Y%m%d_%H%M).log
(output CSVs also go into local_multi_model_ae32_relu_structured_prompt_timing/,
...
MODELS BEING TESTED (already pulled, running on a GTX 3060 GPU):
    ...
    qwen3:14b
(deepseek-r1:8b not tested - removed from Ollama, see local_multi_model_ae32_relu_structured_prompt.py)
...
# "Thinking" models (gemma4:12b, qwen3, ...) generate a <think>...</think>
# block before answering and can burn through the whole 4096-token budget on
# that thinking, getting cut off before it can produce the actual answer.
NUM_CTX = 16384
...
OUTPUT_DIR = Path("local_multi_model_ae32_relu_structured_prompt_timing")
...
#    - IDENTICAL to local_multi_model_ae32_relu_structured_prompt.py, nothing
#      changed here. "time" is still dropped from the AE's numeric input by
#      clean_dataset_dl() - only extract_packet_info() further down reads it
#      from the raw (uncleaned) dataframe for display purposes.
# ============================================================
...
def extract_packet_info(original_packet, df_orig):
    orig_packet_info = {
        ...
        # NEW vs local_multi_model_ae32_relu_structured_prompt.py: the raw
        # wall-clock timestamp (e.g. "16:09:04.703364"), read straight from
        # df_orig - never fed into the AE, display/prompt context only, so
        # the operator can answer "when did this happen?".
        "time": original_packet["time"],
    }
...
# ============================================================
# 2b. STRUCTURED PROMPT WITH TIMING (differs from
#     local_multi_model_ae32_relu_structured_prompt.py: adds a "Time of
#     Occurrence" field + a "Timestamp" line in the packet data)
# ============================================================
...
Time of Occurrence: <the exact timestamp of this packet from the Packet data below, so the operator knows exactly when this happened>
...
--- Packet data ---
Timestamp: {orig_packet_info["time"]}
...
def extract_field(text: str, field_name: str):
    ...
    line, shared by all fields of the structured prompt (Source, Time of
    Occurrence, Likely Cause, Affected Asset, Recommendation, Risk Score)."""
    if not text:
        return None
...
def extract_time_of_occurrence(text: str):
    return extract_field(text, "Time of Occurrence")
...
def check_format_compliance(text: str):
    """Same idea as local_multi_model_ae32_relu_structured_prompt.py, with
    the new "Time of Occurrence" field added to the required set."""
    text = strip_think_block(text)
    fields = {
        "has_source": extract_source(text) is not None,
        "has_time_of_occurrence": extract_time_of_occurrence(text) is not None,
            ...
            for attack_specific, (attack_name, prompt) in prompts.items():
                for repeat_idx in range(REPEATS_PER_PROMPT):
                    ...
                    if output:
                        ...
                        format_fields = {
                            "has_source": False, "has_time_of_occurrence": False,
                            "has_likely_cause": False, "has_affected_asset": False,
                            "has_recommendation": False, "has_risk_score": False,
                    ...
                    results.append({
                        ...
                        "time_of_occurrence": extract_time_of_occurrence(output) if output else None,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare risk-scoring (structured prompt + timing) across several local LLMs on the ICS-SimLab dataset.")
    parser.add_argument(
